...
class VerificationCircuitSynthesis:
    """ Class for training RL agent for verification circuit synthesis task.
    ...
    def run(self, results_folder_name=None):
        ...
        if results_folder_name is None:
            results_folder_name = 'results-vcs/%d/' % (self.env.n_qubits_physical_encoding)
        ...
        for index in range(self.training_config['NUM_AGENTS']):
            ...
            if length < eval_env.max_steps:
                ...
                length += len(eval_env.plus_ancilla_position) * 2
                success += 1
                success_length.append(length)
                circ = stim.Circuit()

                ## Get flag circuit
                for anc in eval_env.plus_ancilla_position:
                        circ.append("H", [anc])
                ...
                ## Save flag circuit
                circ.to_file('%s/circuit_flag_%d.stim' % (results_folder_name, index))

                qc_flag = convert_stim_to_qiskit_circuit(circ)
                qc_flag.draw('mpl', filename='%s/circuit_flag_%d.png' % (results_folder_name, index))

                ## Get encoding and flag circuit
                qc = convert_stim_to_qiskit_circuit(self.env.encoding_circuit, self.env.num_ancillas)
                qc.barrier()
                qc = qc.compose(qc_flag)

                qc.draw('mpl', filename='%s/circuit_combined_%d.png' % (results_folder_name, index))

                combined_circ = self.env.encoding_circuit + circ 
                combined_circ.to_file('%s/circuit_combined_%d.stim' % (results_folder_name, index))
